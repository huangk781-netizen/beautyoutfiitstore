...
- 日本顧客專用頁 `/jp/` 可點進 `/jp/products/<商品 ID>/` 查看商品圖片、影片、日文描述、尺寸、顏色與 Qoo10 商品連結。
- 日本顧客註冊頁 `/jp/register/` 與日文登入頁 `/jp/login/` 仍保留，但日本首頁目前不主動導入會員流程
...
- 日本顧客專用頁目前是商品目錄與 Qoo10 導流用途，不使用本站購物車、本站結帳或日本配送。
- 日本頁價格以商品台幣售價除以 0.21 換算成日幣，再將末兩位調整為 90；超過 90 則降到 90，未超過則補到 90。台幣售價不變。
...
| `ENABLE_BANK_TRANSFER` | `False` | 是否開放新訂單使用 ATM/銀行匯款；目前請維持 `False` |
...
1. 在 `商品分類` 新增分類。
2. 若商品要顯示在日本頁，在分類的「日文分類名稱」填入自然日文，例如 `インナー` 或 `セット`。
3. 在 `商品` 的「商品圖片」欄位按選擇檔案，在檔案選擇器按住 `Ctrl` 一次選取多張不同圖片；第一張會設為主圖。全部合計最多 10 張，`顯示順序` 數字較小者會先顯示。
4. 內衣等需要對照尺寸的商品，可在「尺寸對照」逐行輸入 `尺寸代碼|對照文案`，例如 `XS|32 / 70 A-B 杯`。前台會固定以兩欄顯示，並同步更新尺寸按鈕文字。
5. 日本頁可另外填「日文商品名稱」、「日文商品描述」與「日文尺寸對照」。這些欄位會顯示在 `/jp/` 及日文商品詳情頁，不會改變中文商品頁。
6. 在商品頁的 `商品規格` 新增尺寸、顏色、日文顏色、SKU 與庫存。
7. 確認商品的 `is_active` 已啟用，再到前台檢查。

日本頁商品資訊：

- `/jp/` 會優先顯示日文欄位；日文欄位空白時，才會暫時顯示原本中文資料。
- 日文商品名稱範例：`チューブトップブラ`、`ドット柄3点セット（リボン付き）`
- 日文商品描述建議用日本客人看得懂的自然文案，不要逐字翻譯中文。
- 日文尺寸對照格式與中文相同，每行使用 `尺寸代碼|日文尺寸文案`，例如 `M|着丈 62cm / 身幅 42cm`。
- 完成 Qoo10 上架後，在商品後台的「Qoo10 商品連結」貼上完整網址；日本頁會顯示 `Qoo10で購入` 按鈕。
- 商品尚未填 Qoo10 連結時，日本頁會顯示 `Qoo10販売準備中`，不會導入本站購物車或中文結帳流程。
- `/jp/register/` 可保留作為未來會員收集用途；目前日本首頁不顯示會員註冊按鈕。
