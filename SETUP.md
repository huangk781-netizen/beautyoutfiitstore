...
1. 在 `商品分類` 新增分類。
2. 在 `商品` 的「商品圖片」欄位按選擇檔案，在檔案選擇器按住 `Ctrl` 一次選取多張不同圖片；第一張會設為主圖。全部合計最多 10 張，`顯示順序` 數字較小者會先顯示。
3. 內衣等需要對照尺寸的商品，可在「尺寸對照」逐行輸入 `尺寸代碼|對照文案`，例如 `XS|32 / 70 A-B 杯`。前台會固定以兩欄顯示，並同步更新尺寸按鈕文字。
4. 在商品頁的 `商品規格` 新增尺寸、顏色、SKU 與庫存。
5. 確認商品的 `is_active` 已啟用，再到前台檢查。
